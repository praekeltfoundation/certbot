...
class MarathonAcme(object):
    ...
    def listen_events(self, reconnects=0):
        ...
        def cb_listen(result, reconnects):
            failure = None
            ...
            # If the callback fires, or we get a ResponseFailed error,
            # then the HTTP request to the event stream went fine, but the
            # SSE stream was dropped. Just reconnect for now- if we can't
            # actually connect a different error will occur and be returned.
            self.log.failure(
                'Connection lost listening for events, reconnecting... '
                '({reconnects} so far)',
                reconnects=reconnects, failure=failure, level=LogLevel.warn)
