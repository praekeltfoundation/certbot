...
class TestMarathonAcme(object):
    ...
    def test_sync_acme_server_failure_unknown_prefix(self):
        """
        When a sync is run and we try to issue a certificate for a domain but
        the ACME server returns an error, if that error is of a type that could
        not be determined then a failure should be returned.
        """
        self.fake_marathon.add_app({
            'id': '/my-app_1',
            'labels': {
        ...
        # FIXME: With the acme library v0.10 this error type should be
        # understood
        self.txacme_client.acme_error = acme_Error(
            typ='urn:ietf:params:acme:error:rateLimited', detail='bar')
